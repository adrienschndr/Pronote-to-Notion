...
for id_moyenne in client.periods[2].averages:
    dict_moyennes[str(client.periods[2].averages[id_moyenne].subject.name)] = client.periods[2].averages[
        id_moyenne].student
...
for grade in client.periods[2].grades:
    ...
    properties = {
        ...
        "Matière": {"title": [{"text": {
            "content": (str(subject_dict[grade.subject.name][0]) + " - " + str(dict_moyennes[grade.subject.name]))}}]},
        "Date": {"date": {"start": str(grade.date)}}
    }
    #  Le devoir existe-il déjà dans la base de données ?
    for database_page in pages:
        if database_page["properties"]["Description"]["rich_text"][0]["text"]["content"] == \
